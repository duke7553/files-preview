    ...
    public class WSLDistroManager : ObservableObject
    {
        ...
        public async Task EnumerateDrivesAsync()
        {
            ...
            catch (Exception) // UI Thread not ready yet, so we defer the pervious operation until it is.
            {
                ...
                App.MainWindow.Activated += EnumerateDrivesAsync;
            }
        }

        private async void EnumerateDrivesAsync(object sender, WindowActivatedEventArgs args)
        {
            await SyncSideBarItemsUI();
            App.MainWindow.Activated -= EnumerateDrivesAsync;
        }
                    ...
                    if ((await distroFolder.GetFoldersAsync()).Count != 0)
                    {
                        ...
                        if (App.AppSettings.ShowWslSection && section == null)
                        {
                            section = new LocationItem()
                            {
                                ...
                                Icon = new Microsoft.UI.Xaml.Media.Imaging.BitmapImage(new Uri("ms-appx:///Assets/WSL/genericpng.png")),
